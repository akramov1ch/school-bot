...
@router.message(F.text == "🧑‍💼 Xodim sifatida kirish")
async def employee_entry(message: Message, state: FSMContext, actor_role: UserRole, **_):
    ...
    await message.answer("FX ID ni kiriting (misol: FX12345):", reply_markup=None)
...
async def employee_wait_password(message: Message, state: FSMContext, **_):
    ...
    try:
        ...
        if user.role == UserRole.TEACHER:
            await message.answer("✅ Kirish muvaffaqiyatli.", reply_markup=teacher_menu_kb())
        elif user.role == UserRole.CASHIER:
            await message.answer("✅ Kirish muvaffaqiyatli.", reply_markup=cashier_menu_kb())
        elif user.role == UserRole.HR:
            await message.answer("✅ Kirish muvaffaqiyatli.", reply_markup=hr_menu_kb())
        elif user.role == UserRole.ADMIN:
            await message.answer("✅ Kirish muvaffaqiyatli.", reply_markup=admin_menu_kb())
        else:
